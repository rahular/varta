...
MODEL_DIR=models/mbert_latin_run3
...
for lg in as bh bn en gu hi kn ml mr ne or pa ta te ur; do
    python run_sum.py   --model_name_or_path=${MODEL_DIR}/checkpoint-3000 \
                        --text_column=text \
                        --summary_column=headline \
                		--lang_prefix=true\
                		--lang_code_column=langCode\
                        --test_file=${TEST_DIR}/test_${lg}_100k_en.json \
